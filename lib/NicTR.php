<?php
namespace WHMCS\Module\Registrar\NicTR;
use Exception;
...
class NicTR {
    ...
    public function error($message, $raw = '') {
        ...
        //TODO: custom exception yazilacak
                    ...
                    if (strlen($form_params['clientid']) > 5 && strlen($form_params['hash']) > 5 ) {
                        if ($this->ccValidate( $cc_params ) ) {
                            ...
                            if (strstr($pay_html,'Ödeme işleminiz tamamlanmıştır')) {
                                ...
                                if (strpos($pay_html, 'Kredi kartı sahibi bilgisi geçersiz.')) {
                                    $this->error('Kredi kartı sahibi bilgisi geçersiz.');
                                }else if(strpos($pay_html, 'Kredi kartı numarası geçersiz.')) {
                                    $this->error('Kredi kartı numarası geçersiz.', "kredi karti gecersiz\n");
                                }else if(strpos($pay_html, 'Lütfen geçerli bir son kullanma tarihi giriniz.')) {
                                    $this->error('Lütfen geçerli bir son kullanma tarihi giriniz.');
                                }else{
                                    $this->error('Domain yenileme sırasında bilinmeyen bir hata meydana geldi', $pay_html);
                                }
                            }
    ...
    public function ccValidate($cc_params) {
        if (strlen($cc_params['ccOwner']) > 3 &&
            strlen($cc_params['pan']) == 16 && is_numeric($cc_params['pan']) && 
            strlen($cc_params['cv2']) == 3 && 
            ($cc_params['ccIssuerId'] == 1 || $cc_params['ccIssuerId'] == 2) && 
            strlen($cc_params['Ecom_Payment_Card_ExpDate_Month'] ) == 2 && is_numeric($cc_params['Ecom_Payment_Card_ExpDate_Month']) && 
            strlen($cc_params['Ecom_Payment_Card_ExpDate_Year']) == 4 && is_numeric($cc_params['Ecom_Payment_Card_ExpDate_Year']) 
        ) {
            return true;
        }else{
            return false;
